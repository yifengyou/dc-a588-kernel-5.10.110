		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		gpiod_set_value_cansleep(mc_data->spk_ctl_gpio, 1);//default open two track voice,so open all gpio of voice
		break;
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		gpiod_set_value_cansleep(mc_data->hp_ctl_gpio, 1);//default open two track voice,so open all gpio of voice
		break;
	case SND_SOC_DAPM_PRE_PMD:
		gpiod_set_value_cansleep(mc_data->spk_ctl_gpio, 0);
		gpiod_set_value_cansleep(mc_data->hp_ctl_gpio, 0);
