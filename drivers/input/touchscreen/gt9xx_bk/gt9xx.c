                                ...
                                struct goodix_ts_data *ts)
{
    ...
    ts->input_dev->phys = goodix_ts_name;//phys;// this is bug, you can set it as NULL too, fix it by zts 2023.5.3
...
*******************************************************/
#if 1
//#include <u_pan_data.h>
static int goodix_ts_thread_probe(void *data)
{   
    s32 ret = -1;
    struct goodix_ts_data *ts;
    u16 version_info;
    //struct ztl_upan_dev* pztl_upan_dev = NULL;
    struct i2c_client *client = i2c_connect_client;
    struct device_node *np = NULL;
    enum of_gpio_flags rst_flags, pwr_flags;
    u32 val;
        printk("%s() thread start\n", __func__);
    if(i2c_connect_client == NULL)return -1;
    ...
    if (of_property_read_u32(np, "tp-size", &val)) {
        ...
        gtp_x_reverse = FALSE;
        gtp_y_reverse = FALSE;
	pztl_upan_dev = get_zts_upan_data();
	if(pztl_upan_dev!= NULL && (pztl_upan_dev->gtp_used == 1)){
        //zts   
		if(pztl_upan_dev->gtp_max_x > 100){
			ts->abs_x_max = pztl_upan_dev->gtp_max_x ;
		}
		if(pztl_upan_dev->gtp_max_y > 100){
			ts->abs_y_max = pztl_upan_dev->gtp_max_y ;
		}
		if(pztl_upan_dev->gtp_x_reverse_enable == 1){
			gtp_x_reverse = TRUE ;
		}
		if(pztl_upan_dev->gtp_y_reverse_enable == 1){
			gtp_y_reverse = TRUE ;
		}
		if(pztl_upan_dev->gtp_swap_x2y == 1){
			gtp_change_x2y = TRUE ;
		}
	}	
#endif
...
static struct task_struct * init_tsk;
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	i2c_connect_client = client;
	init_tsk = kthread_run(goodix_ts_thread_probe,NULL,"goodix_ts_thread_probe");
	return 0;
//goodix_ts_thread_probe
}
